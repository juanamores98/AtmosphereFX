using ICities;
namespace AtmosphereFX.Options
{
    internal static class OptionsPanel
    {
        internal static void Build(UIHelperBase helper)
        {
            var group = helper.AddGroup("AtmosphereFX");
            group.AddButton("VANILLA", FxModule.Release);
            group.AddButton("OPTIMIZED / Default", FxModule.ApplyOptimized);
            group.AddButton("Open compact panel", () => FxModule.OpenStandalone());
            group.AddCheckbox("Apply saved settings when a city loads", Config.ModConfig.ApplyOnLoad, value => { Config.ModConfig.ApplyOnLoad = value; Config.ConfigStore.Save(true); });
        }
    }
}
